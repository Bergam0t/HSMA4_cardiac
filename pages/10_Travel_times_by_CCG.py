...
def create_ics_df(df_activity, ics_routino_filename, prov_gdf):
    ...
    df_ics = df_ics[
        ["Der_Provider_Site_Code", "Patient_LSOA", "Travel_distance_km", "Travel_time"]
    ]
...
def plot_times_impact_quick(
    ...
    gdf.plot(ax=ax, column="Time_change", legend=True, cax=cax)
...
def plot_new_prov_times_quick(
    ...
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="2%", pad=0.1)
    gdf.plot(ax=ax, column="new_min_time", legend=True, cax=cax)
    test_prov_gdf = prov_gdf[
        prov_gdf["Provider_Site_Code"].isin(current_providers + new_provider_list)
    ]
    test_prov_gdf.plot(
...
def get_national_time_map(ccg_mapping_filename, activity_data_minimal_filename):
    ...
    df_activity = pd.merge(df_activity, df_ccg, on="Pt_CCG_Code", how="left").set_index(
        df_activity.index
    )
    df_activity["CCG_Code_2122"] = df_activity["CCG_Code_2122"].combine_first(
        df_activity["Pt_CCG_Code"]
    )
    df_activity["CCG_2122"] = df_activity["CCG_2122"].combine_first(
        df_activity["Pt_CCG"]
    )
    ccg_gdf = gpd.read_file(
        "zip://./Clinical_Commissioning_Groups_(April_2021)_EN_BUC.zip"
    )
    ccg_gdf = ccg_gdf.rename(
        {"CCG21NM": "Pt_CCG", "Shape__Are": "CCG_Area_sq_km"}, axis="columns"
    )
    ccg_gdf["CCG_Area_sq_km"] *= 10**-6
    df_ccg_pop = pd.read_csv("ccg_pop.csv", encoding="unicode_escape")
    df_ccg_pop = df_ccg_pop.rename(
        {
            "CCG Name": "Pt_CCG",
            "STP21 Name": "Pt_ICS",
    ...
    df_ccg_pop["Population"] = (
        df_ccg_pop["Population"].str.replace(",", "").astype(float)
    )
    ccg_gdf = pd.merge(ccg_gdf, df_ccg_pop, on="Pt_CCG", how="left").set_index(
        ccg_gdf.index
    )
    ccg_gdf["Population_density"] = ccg_gdf["Population"] / ccg_gdf["CCG_Area_sq_km"]
    ccg_gdf["CCG_2122"] = ccg_gdf["Pt_CCG"].str.upper()
    df = (
        df_activity[["CCG_2122", "Travel_distance_km", "Travel_time"]]
        .groupby("CCG_2122")
        .agg({"Travel_distance_km": "mean", "Travel_time": "mean"})
        .reset_index()
    )
...
def get_national_providers(national_provider_filename, activity_data_minimal_filename):
    ...
    df_act_by_prov = (
        df_activity[["Der_Provider_Site_Code"]]
        .groupby("Der_Provider_Site_Code")
        .agg(
            {
                "Der_Provider_Site_Code": "count",
            }
        )
    )
    df_act_by_prov = df_act_by_prov.rename(
        columns={"Der_Provider_Site_Code": "count_site"}
    )
    df_act_by_prov = df_act_by_prov[df_act_by_prov["count_site"] > 10]
    nat_prov10_gdf = pd.merge(
        nat_prov_gdf,
        df_act_by_prov,
...
nat_act_gdf = get_national_time_map(
    ccg_mapping_filename, activity_data_minimal_filename
)
nat_prov10_gdf = get_national_providers(
    national_provider_filename, activity_data_minimal_filename
)
...
nat_act_gdf["Pop_density_rank"] = nat_act_gdf["Population_density"].rank(
    ascending=False
)
...
st.markdown(
    "#### A comparison of possible configurations of sites offering "
    + "this service, looking at the potential impact on travel times "
    + "and distances"
)

st.markdown(
    "We first look at a sample of our data - all elective cardiac "
...
st.write(
    "Nationally, based on Routino data, we see a mean travel time of "
...
st.markdown(
    "We map the travel times based on Routino data for the patient "
...
if not os.path.exists(os.getcwd() + "/output/nat_map_ccg.png"):
    ...
    # Plot location of hospitals
    nat_prov10_gdf.plot(ax=ax, edgecolor="k", facecolor="w", markersize=200, marker="*")
...
st.write(
    "Looking at travel times by CCG, we note a negative "
    + "correlation between population density and mean travel time. "
    + ""
)

with st.expander("Ranked travel time and population density by CCG", expanded=False):
    st.dataframe(
        nat_act_gdf[
            [
                "CCG_2122",
...
size_mapper = LinearInterpolator(
    x=[df.Population.min(), df.Population.max()], y=[5, 20]
)
...
fig.circle("x", "y", size={"field": "desc2", "transform": size_mapper}, source=source)
...
st.write(
    "We calculate Spearman's rank correlation coefficient to show "
...
st.write(
    "We observe that Kent and Medway CCG sees travel times "
...
if not os.path.exists(os.getcwd() + "/output/kent_prov.png"):
    f = plot_proposed_sites1(
        km_prov_gdf, km_lsoa_gdf, "Proposed Sites in Kent and Medway"
    )
    fig, ax = f
