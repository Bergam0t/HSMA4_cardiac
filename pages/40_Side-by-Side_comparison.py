...
def create_ics_df(df_activity, ics_routino_filename, prov_gdf):
    # filter to ICS data
    df_ics = df_activity[
        df_activity["Patient_LSOA"].isin(list(ics_lsoa["yr2011_LSOA"]))
    ]
    df_ics = df_ics[
        [
...
def test_sites_quick(
    df_actuals_augmented, df_activity, d_prov, prov_gdf, new_prov_list
):
    ...
    prop_reduced = (
        sum(df_test["Time_new_sites_min"] < df_test["Time_original"])
        / df_test["Time_new_sites_min"].count()
    )
    ...
    prop_reduced_natl_med = (
        sum(df_test["Time_min"] < nat_median_time) / df_test["Time_min"].count()
    )
    # Original mean time
    ...
    total_time_reduction = (df_test["Time_original"] - df_test["Time_min"])[
        df_test["Time_new_sites_min"] < df_test["Time_original"]
    ].sum()
    # Time reduction per spell
    time_reduction_per_spell = (
        total_time_reduction / df_test["Time_new_sites_min"].count()
    )
    # Total travel distance reduction (over 70 months), value in km travelled
    total_dist_reduction = (df_test["Distance_original"] - df_test["Distance_min"])[
        df_test["Time_min"] < df_test["Time_original"]
    ].sum()

    df_to_add = pd.DataFrame(
        {
            "Added_providers": [",".join(new_prov_list)],
...
def gdf_add_site_list(ics_routino_filename, shape_filename, _prov_gdf, test_prov_list):
    ...
    for test_prov in test_prov_list:
        right = df_ics_routino[df_ics_routino["to_postcode"] == prov_dict[test_prov]][
            ["from_postcode", "to_postcode", "time_min", "distance_km"]
        ]
        gdf_test = pd.merge(
            left=gdf_test,
            right=right,
        ...
        gdf_test = gdf_test.rename(
            columns={
                "time_min": "time_" + test_prov,
...
def plot_times_impact_quick(
    ...
    plt.title(
        "Impact on travel times with the addition of sites\n " + "\n".join(sites),
        fontsize=16,
    )
    ax.set_axis_off()
    ...
    test_prov_gdf = prov_gdf[
        prov_gdf["Provider_Site_Code"].isin(current_providers + new_provider_list)
    ]
    test_prov_gdf.plot(
        ax=ax, edgecolor="r", facecolor="silver", markersize=200, marker="*"
    )
    if save_output:
        ...
        plt.savefig(
            os.getcwd()
            + "/output/time_impact_maps/"
            + "-".join(new_provider_list)
            + ".png"
        )
    else:
        pass
...
def plot_time_impact_threshold_quick(
    lsoa_to_all_gdf,
    current_providers,
    new_provider_list,
    prov_gdf,
    ...
    gdf["Orig_<_nat"] = np.where(
        gdf[current_list].min(axis=1) <= threshold, True, False
    )
    gdf["New_<_nat"] = np.where(
        gdf[test_prov_list].min(axis=1) <= threshold, True, False
    )
    gdf["Compare_with_national"] = np.where(
        gdf["Orig_<_nat"],
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    gdf.plot(ax=ax, column="Compare_with_national", legend=True)
    ax.set_axis_off()
    ax.set_title(
        "Impact on travel time compared to national median for site\n "
        + "\n".join(sites),
        fontsize=16,
    )
    test_prov_gdf = prov_gdf[
        prov_gdf["Provider_Site_Code"].isin(current_sites + new_provider_list)
    ]
    test_prov_gdf.plot(
        ax=ax, edgecolor="r", facecolor="silver", markersize=200, marker="*"
    )
...
def get_summary_table(
    _prov_gdf, current_sites, df_activity, d_prov, site_pairs, save_output
):
    ...
    for pair in site_pairs:
        df_to_add = test_sites_quick(
            df_actuals_augmented, df_activity, d_prov, prov_gdf, pair
        )
        df_results = pd.concat([df_results, df_to_add])
    for site in new_provider_list:
        df_to_add = test_sites_quick(
            df_actuals_augmented, df_activity, d_prov, prov_gdf, [site]
        )
        df_results = pd.concat([df_results, df_to_add])
    if save_output:
        ...
        pass
    df_results["Number_spells_reduced_time"] = df_results[
        "Number_spells_reduced_time"
    ].astype(int)
    return df_results
...
def get_actuals_augmented(df_ics, ics_routino_filename, test_prov_list):
    df_test = df_ics.copy()
    df_ics_routino = pd.read_csv(ics_routino_filename)
    for test_prov in test_prov_list:
        right = df_ics_routino[df_ics_routino["to_postcode"] == d_prov[test_prov]][
            ["from_postcode", "to_postcode", "time_min", "distance_km"]
        ]
        df_test = pd.merge(
            left=df_test, right=right, left_on="Patient_LSOA", right_on="from_postcode"
        )
...
def kde_plot_quick(df_actuals_augmented, test_prov_list, threshold, save_output):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sbn.kdeplot(
        ax=ax, data=df_test["Time_original"], clip=(0, 125), fill=True, legend=True
    )
    plt.axvline(
        x=df_test["Time_original"].median(), linewidth=2, color="cornflowerblue"
    )
    ...
    plt.axvline(x=df_test["Time_new_config_min"].median(), linewidth=2, color="orange")
    plt.axvline(x=threshold, linewidth=2, color="g")
    ...
    if save_output:
        ...
        plt.savefig(
            os.getcwd() + "/output/kde_plots_mult/" + "-".join(test_prov_list) + ".png"
        )
    else:
        pass
    return fig, ax
...
def plot_new_prov_times_quick(
    lsoa_to_all_gdf, current_providers, new_provider_list, save_file
):
    current_list = ["time_" + a for a in current_providers]
    test_prov_list = ["time_" + prov for prov in new_provider_list]
    gdf = lsoa_to_all_gdf.copy()
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    plt.title(
        "Travel times with the addition of sites:\n " + "\n".join(sites), fontsize=16
    )
    ax.set_axis_off()
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="2%", pad=0.1)
    ...
    if save_file:
        ...
        plt.savefig(
            os.getcwd() + "/output/time_plots/" + "-".join(new_provider_list) + ".png"
        )
    else:
        pass
    return fig, ax
...
lsoa_to_all_gdf = gdf_add_site_list(
    ics_routino_filename, shape_filename, prov_gdf, full_list
)
...
km_lsoa_gdf = filter_lsoa_to_ics(
    shape_filename, ics_filename, "lsoa11cd", "yr2011_LSOA"
)

site_dict = dict(
    zip(km_prov_gdf["Provider_Site_Name"], km_prov_gdf["Provider_Site_Code"])
)
site_dict2 = dict(
    zip(km_prov_gdf["Provider_Site_Code"], km_prov_gdf["Provider_Site_Name"])
)
...
site_list.sort()
df_results = get_summary_table(
    prov_gdf, current_sites, df_activity, d_prov, site_pairs, save_output
)
...
st.write(
    "Select configurations to compare in the 2 select boxes in the "
...
with st.sidebar:
    with st.form("select_sites_form"):
        selected_site_pair1 = st.multiselect(
            "First configuration for comparison:", site_list, default=[site_list[0]]
        )
        selected_site_pair2 = st.multiselect(
            "Second configuration for comparison:", site_list, default=[site_list[-1]]
        )
...
with col1:
    ...
    st.pyplot(fig)
    # new time map
    f = plot_new_prov_times_quick(lsoa_to_all_gdf, current_sites, sites, False)
    fig, ax = f
    st.pyplot(fig)
    # time impact map
    f = plot_times_impact_quick(
        lsoa_to_all_gdf, current_sites, sites, prov_gdf, threshold, save_output
    )
    fig, ax = f
    st.pyplot(fig)
    # impact relative to threshold map
    f = plot_time_impact_threshold_quick(
        lsoa_to_all_gdf, current_sites, sites, prov_gdf, threshold, save_output
    )
    fig, ax = f
    st.pyplot(fig)
    ...
    if len(sites) == 1:
        ...
        df_results_one = df_results_one[
            df_results_one["Added_providers"].str.len() == 5
        ]
        ...
        df_results_one["New_median_time_rank"] = df_results_one["New_median_time"].rank(
            method="min", ascending=True
        )
        new_med_time_dict = dict(
            zip(
                df_results_one["Added_providers"],
                df_results_one["New_median_time_rank"],
            )
        )
        new_med_time_rank = new_med_time_dict[site]

        proportion_reduced = df_results.loc[site]["Proportion_spells_reduced_time"]
        df_results_one["Prop_reduced_rank"] = df_results_one[
            "Proportion_spells_reduced_time"
        ].rank(method="min", ascending=False)
        prop_reduced_dict = dict(
            zip(df_results_one["Added_providers"], df_results_one["Prop_reduced_rank"])
        )
        prop_reduced_rank = prop_reduced_dict[site]

        proportion_spells_under_natl_median = df_results.loc[site][
            "Proportion_spells_under_natl_median"
        ]
        df_results_one["Prop_under_nat_median_rank"] = df_results_one[
            "Proportion_spells_under_natl_median"
        ].rank(method="min", ascending=False)
        prop_under_nat_median_dict = dict(
            zip(
                df_results_one["Added_providers"],
                df_results_one["Prop_under_nat_median_rank"],
            )
        )
        ...
        df_results_one["Time_reduction_rank"] = df_results_one[
            "Time_reduction_per_spell"
        ].rank(method="min", ascending=False)
        time_reduction_dict = dict(
            zip(
                df_results_one["Added_providers"], df_results_one["Time_reduction_rank"]
            )
        )
        time_reduction_rank = time_reduction_dict[site]

        total_distance_reduction = df_results.loc[site]["Total_distance_reduction"]
        df_results_one["Dist_reduction_rank"] = df_results_one[
            "Total_distance_reduction"
        ].rank(method="min", ascending=False)
        dist_reduction_dict = dict(
            zip(
                df_results_one["Added_providers"], df_results_one["Dist_reduction_rank"]
            )
        )
        dist_reduction_rank = dist_reduction_dict[site]

        new_max_time = df_results.loc[site]["New_maximum_time"]
        df_results_one["Max_time_rank"] = df_results_one["New_maximum_time"].rank(
            method="min", ascending=True
        )
        max_time_dict = dict(
            zip(df_results_one["Added_providers"], df_results_one["Max_time_rank"])
        )
        max_time_rank = max_time_dict[site]

        st.markdown(
            "#### Impact of additional site at {} on key travel time metrics".format(
                selected_site_pair1[0]
            )
        )

        st.markdown(
            f"* Median travel time for Kent and Medway patients reduced "
        ...
        st.markdown(
            f"* Travel times would be reduced for {100 * proportion_reduced:.0f}% "
        ...
        st.markdown(
            "* Travel times would be reduced to less than the national median "
        ...
        st.markdown(
            f"* Mean travel time reduction would be {mean_time_reduction:.0f} "
            + f"minutes. This configuration ranks {time_reduction_rank:.0f} "
            + f"out of {count_options} for this metric."
        )

        st.markdown(
            f"* Total travel distance (1 way) reduction would be {total_distance_reduction:,.0f} "
            + f"km for the period of actuals considered. This configuration ranks {dist_reduction_rank:.0f} "
            + f"out of {count_options} for this metric."
        )

        st.markdown(
            f"* The new maximum travel time would be {new_max_time:.0f} minutes. "
        ...
        df_results_two = df_results_two[
            df_results_two["Added_providers"].str.len() == 11
        ]
        count_options = len(df_results_two)

        med_orig = df_results.loc[site]["Original_median_time"]
        med_new = df_results.loc[site]["New_median_time"]
        df_results_two["New_median_time_rank"] = df_results_two["New_median_time"].rank(
            method="min", ascending=True
        )
        new_med_time_dict = dict(
            zip(
                df_results_two["Added_providers"],
                df_results_two["New_median_time_rank"],
            )
        )
        new_med_time_rank = new_med_time_dict[site]

        proportion_reduced = df_results.loc[site]["Proportion_spells_reduced_time"]
        df_results_two["Prop_reduced_rank"] = df_results_two[
            "Proportion_spells_reduced_time"
        ].rank(method="min", ascending=False)
        prop_reduced_dict = dict(
            zip(df_results_two["Added_providers"], df_results_two["Prop_reduced_rank"])
        )
        prop_reduced_rank = prop_reduced_dict[site]
        ...
        df_results_two["Prop_under_nat_median_rank"] = df_results_two[
            "Proportion_spells_under_natl_median"
        ].rank(method="min", ascending=False)
        prop_under_nat_median_dict = dict(
            zip(
                df_results_two["Added_providers"],
                df_results_two["Prop_under_nat_median_rank"],
            )
        )
        prop_under_nat_median_rank = prop_under_nat_median_dict[site]

        mean_time_reduction = df_results.loc[site]["Time_reduction_per_spell"]
        df_results_two["Time_reduction_rank"] = df_results_two[
            "Time_reduction_per_spell"
        ].rank(method="min", ascending=False)
        time_reduction_dict = dict(
            zip(
                df_results_two["Added_providers"], df_results_two["Time_reduction_rank"]
            )
        )
        time_reduction_rank = time_reduction_dict[site]

        total_distance_reduction = df_results.loc[site]["Total_distance_reduction"]
        df_results_two["Dist_reduction_rank"] = df_results_two[
            "Total_distance_reduction"
        ].rank(method="min", ascending=False)
        dist_reduction_dict = dict(
            zip(
                df_results_two["Added_providers"], df_results_two["Dist_reduction_rank"]
            )
        )
        dist_reduction_rank = dist_reduction_dict[site]

        new_max_time = df_results.loc[site]["New_maximum_time"]
        df_results_two["Max_time_rank"] = df_results_two["New_maximum_time"].rank(
            method="min", ascending=True
        )
        max_time_dict = dict(
            zip(df_results_two["Added_providers"], df_results_two["Max_time_rank"])
        )
        max_time_rank = max_time_dict[site]
        ...
        st.markdown(
            f"* Travel times would be reduced to less than the national median "
            + f"for {100 * proportion_spells_under_natl_median:.0f}% "
            + "of the Kent and Medway patients, based on historic activity. "
            + f"This configuration ranks {prop_under_nat_median_rank:.0f} out of "
            + f"{count_options} for this metric."
...
with col2:
    ...
    if len(sites) == 1:
        ...
        st.markdown(
            "#### Impact of additional site at {} on key travel time metrics".format(
                selected_site_pair2[0]
            )
        )

        st.markdown(
            f"* Median travel for Kent and Medway patients reduced "
            + f"from {med_orig:.0f} to {med_new:.0f} minutes. "
            + f"This configuration ranks {new_med_time_rank:.0f} out of "
            + f"{count_options} for this metric."
        )
...
if len(sites) == 0:
    st.write(
        "The maps and chart above show travel times with no additional "
    ...
    if count_metric1 > count_metric2:
        ...
        st.markdown(f"## {selected_site_pair1[0]}")
        st.markdown(
            f"### is superior in {count_metric1} out of {len(rank_list1)} metrics."
        )
        ...
        st.markdown(f"## {selected_site_pair2[0]}")
        st.markdown(
            f"### is superior in {count_metric2} out of {len(rank_list2)} metrics."
        )
